...
export default function EbookCreateForm() {
  ...
  const handleCoverImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData(prev => ({ ...prev, coverImagePath: data.filename }))
      } else {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const formDataUpload = new FormData()
      formDataUpload.append('ebook', file)
      ...
      if (response.ok) {
        const data = await response.json()
        setFormData(prev => ({ ...prev, filePath: data.filename }))
      } else {
        alert('Upload file e-book gagal')
      }
    } catch (error) {
      console.error('Upload error:', error)
      alert('Upload file e-book gagal')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.coverImagePath || !formData.filePath) {
      alert("Harap upload gambar sampul dan file e-book terlebih dahulu")
      return
    }
    ...
    try {
      ...
      alert("Terjadi kesalahan saat membuat e-book")
    } finally {
      setLoading(false)
    }
  }
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="ebookFile">File E-Book *</Label>
                <Input
                  id="ebookFile"
                  type="file"
                  accept=".pdf"
                  ...
                  required
                />
                {uploadingFile && <p className="text-sm text-muted-foreground">Mengupload file...</p>}
                {formData.filePath && <p className="text-sm text-green-600">✓ File berhasil diupload</p>}
              </div>
            </div>
